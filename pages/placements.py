...
st.set_page_config(page_title="Placements Analysis", layout="wide")
...
with col2:
    # Placeholder for the prefix selectbox (left side before the hyphen)
...
if run_clicked:
    ...
    if len(prefixes) == 0:
        ...
        default_prefix = prefix_counts.index[0]
        with col2:
    ...
    if user_suffix.strip() and selected_prefix:
        ...
        candidates = df.loc[df["_prefix"] == selected_prefix, "_suffix"].dropna().unique().tolist()
        if len(candidates) == 0:
    ...
    with st.expander("Peek data and schema"):
        ...
        # Note: DataFrame.info() prints to stdout; using capture via to_string for display
        buf = []
        df.info(verbose=True, memory_usage="deep", buf=buf.append)
        st.text("\n".join(buf))
    ...
    if "Date" in df.columns:
        ...
        total_daily_spend = spend_by_day_placement.groupby("Date")["Spend"].sum().reset_index().rename(columns={"Spend": "Total Daily Spend"})
        ...
        if camp_query.strip():
            ...
            if candidates:
                ...
                if not selected_campaign_data.empty and "Date" in selected_campaign_data.columns:
                    ...
                    st.plotly_chart(fig_clicks_by_day, use_container_width=True)
